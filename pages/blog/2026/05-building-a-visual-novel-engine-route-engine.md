...
In the system state, we have an array to track pending side effects. The system checks for pending side effects and handles them.

- `'render'` side effects: call Route Graphics render with the latest `renderState`
- Saving data: save to persistent storage like IndexedDB or online

There's some manual work to connect the environment, system store, and side effects. For delayed actions, we use the timer.

### Full Example 1: Click to Next Line

The screen has a base with click actions for `nextLine`:

1. When you click, Route Graphics emits event
2. We handle the event, call Route Engine to run action `nextLine`
3. Next line does calculations, updates the current line
4. We call render and get updated `presentationState` & `renderState`
5. Action will create a side effect
6. Screen is updated
7. pending side effect is removed.
...
Route Engine is a well-designed library serving as a building block for building Visual Novel applications.

The core engine is surprisingly compact. Less than 5000 lines of JavaScript. This small size is a hint that we've found a good design and architecture.
