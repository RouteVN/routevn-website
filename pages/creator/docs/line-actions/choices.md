...
Pick the [choice layout](/creator/docs/layouts/#choice) used to display the options. Only layouts with the `Choice` type are shown here.
...
Use `Move to Section` when the choice should jump to another section. Select the scene and section to use as the target. This action uses the same target behavior as [Section Transition](/creator/docs/line-actions/section-transition/).
